...
import reflex as rx
from reflex.components.recharts import bar_chart, bar, x_axis, y_axis, cartesian_grid, tooltip, legend, responsive_container
...
pedidos_data = [{"name": "Botas", "count": 150}, {"name": "Suministros", "count": 600}, {"name": "Equipos", "count": 200}]
criticos_data = [
...
def metric_card(title, value, icon, color):
    return rx.hstack(
        ...
        padding="8px", background_color="#141416", border="1px solid #222225", border_radius="6px", width="100%"
    )
...
def world_clock(location, time_str, date_str):
    return rx.vstack(
        ...
        padding="10px", background_color="#141416", border="1px solid #222225", border_radius="8px", text_align="center", width="100%"
    )
...
def system_performance():
    return rx.vstack(
        ...
        rx.hstack(rx.icon(tag="activity", color="green", size=14), rx.text("Rendimiento del Sistema", color="white", size="1"), spacing="2"),
        rx.grid(
        ...
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%", spacing="2", align_items="start"
    )

def ticket_queue():
    return rx.vstack(
        rx.heading("SERVICE TICKET QUEUE", size="2", color="gray"),
        rx.hstack(rx.icon(tag="ticket", color="yellow", size=14), rx.heading("Tickets Activos", size="1", color="white"), spacing="2"),
        rx.vstack(
            *[ticket_status_item(t["status"], t["count"], t["color"], "requieren atención" if t["status"] == "Pendientes" else "siendo atendidos" if t["status"] == "En Proceso" else "esta semana") for t in tickets_data],
            spacing="2", width="100%"
        ),
        rx.divider(border_color="#222225", margin_y="4px"),
        rx.text("Tickets por Prioridad", size="1", color="gray", weight="bold"),
        rx.vstack(
            rx.hstack(rx.text("Alta", size="1", width="40px"), rx.progress(value=15, color_scheme="red", width="100%"), rx.text("3", size="1", color="gray")),
            rx.hstack(rx.text("Media", size="1", width="40px"), rx.progress(value=35, color_scheme="yellow", width="100%"), rx.text("7", size="1", color="gray")),
            rx.hstack(rx.text("Baja", size="1", width="40px"), rx.progress(value=50, color_scheme="blue", width="100%"), rx.text("10", size="1", color="gray")),
            width="100%", spacing="2"
        ),
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%", spacing="2", align_items="start"
    )
...
def resolution_metrics():
    return rx.vstack(
        ...
        rx.grid(
            ...
            columns="2", width="100%", spacing="2"
        ),
        rx.divider(border_color="#222225", margin_y="2px"),
        rx.hstack(rx.icon(tag="alert_triangle", color="#ef4444", size=12), rx.text("Críticos a Atender", size="1", color="#ef4444", weight="bold"), spacing="2"),
        rx.vstack(
            *[rx.hstack(rx.badge(item["id"], bg=item["color"], color="white", size="1"), rx.text(item["titulo"], size="1", color="white", width="140px", is_truncated=True), rx.spacer(), rx.text(item["tiempo"], size="1", color="gray"), width="100%") for item in criticos_data],
            spacing="2", width="100%"
        ),
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%", spacing="2", align_items="start"
    )
...
def world_map_section():
    return rx.vstack(
        ...
        rx.box(
            ...
            width="100%", background_color="#111113", border="1px solid #222225", border_radius="8px", overflow="hidden"
        ),
        rx.flex(
            ...
            spacing="3", justify="center", width="100%", wrap="wrap"
        ),
        rx.grid(
        ...
        rx.vstack(
            rx.hstack(rx.icon(tag="terminal", color="#FFD700", size=14), rx.text("CONSOLA DE EVENTOS EN VIVO", size="1", color="white", weight="bold"), spacing="2"),
            rx.vstack(
                rx.foreach(
            ...
            width="100%", min_height="125px", padding="12px", background_color="#0d0d0e", border="1px solid #222225", border_radius="6px", align_items="start"
        ),
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%", spacing="3"
    )
    ...
    return rx.vstack(
        ...
        responsive_container(
            bar_chart(
                ...
                x_axis(data_key="name", stroke="gray", style={"fontSize": 10}),
                y_axis(stroke="gray", style={"fontSize": 10}),
        ...
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%"
    )
        ...
        rx.vstack(
            *[
                rx.hstack(
                    ...
                    width="100%", padding="6px", border_bottom="1px solid #222225"
                ) for veh in flota_data
            ],
            spacing="1", width="100%"
        ),
        padding="14px", background_color="#111113", border="1px solid #222225", border_radius="8px", width="100%", align_items="start"
    )

# --- Al final de sistema/components/stock_dashboard.py ---
